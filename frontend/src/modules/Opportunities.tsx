...
import { api, Contact, money, Opportunity, PipelineStage, shortDate } from '../api'
import { Modal } from '../components/Modal'
import { Page } from '../components/Page'
import { EmptyState, ErrorState, LoadingState } from '../components/States'

const defaultStages = ['new', 'qualified', 'proposal', 'won', 'lost']
...
export function Opportunities() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    setError('')
    Promise.all([api<{ opportunities: Opportunity[] }>('/api/v1/opportunities'), api<{ contacts: Contact[] }>('/api/v1/contacts'), api<{ stages: PipelineStage[] }>('/api/v1/pipeline-stages')])
      .then(([opportunities, people, pipeline]) => { setItems(opportunities.opportunities); setContacts(people.contacts); setStages(pipeline.stages.map((stage) => stage.id)) })
