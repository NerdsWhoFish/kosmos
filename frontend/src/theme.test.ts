...
describe("mobile form accessibility", () => {
  ...
  it("stacks panel labels and actions instead of squeezing fields together", () => {
    expect(compactStyles).toContain(
      ".panel form label { min-width: 0; display: grid;",
    );
    expect(compactStyles).toContain(
      ".panel .form-actions { grid-template-columns: 1fr; }",
    );
    expect(compactStyles).toContain(
      ".lead-intake-form label { min-width: 0; display: grid;",
    );
    expect(compactStyles).toContain(
      '.document-upload input[type="file"] { width: 100%; min-width: 0;',
    );
  });
});
