...
export function Overview({ firstName, navigate }: { firstName: string; navigate: (path: string) => void }) {
  ...
  const load = useCallback(() => {
    setLoading(true)
    setError('')
    Promise.all([api<Summary>('/api/v1/summary'), api<Landing>('/api/v1/landing'), api<{ notifications: Landing['notifications'] }>('/api/v1/notifications')])
      .then(([nextSummary, nextLanding, notificationResult]) => { setSummary(nextSummary); setLanding({ ...nextLanding, notifications: notificationResult.notifications }) })
  ...
  return <>
    ...
    <section className="stats-row">
      ...
      <Stat label="This month's costs" value={money(summary.currentMonthCostCents)} detail="Recorded business spending" tone="green" onClick={() => navigate('/operations')} />
    </section>
    ...
    {!!landing.notifications.length && <section className="panel notification-panel"><div className="panel-heading"><div><p className="eyebrow">Across your business</p><h2>What just happened</h2></div><button className="text-button" onClick={() => navigate('/communications')}>Open inbox <ArrowUpRight size={15} /></button></div><div className="activity-list">{landing.notifications.slice(0, 5).map((item) => <button className="activity-item notification-item" key={item.id} onClick={() => navigate(item.href)}><span className="activity-icon lavender"><Mail size={16} /></span><span><strong>{item.title}</strong><small>{item.summary}</small><time>{new Intl.RelativeTimeFormat('en', { numeric: 'auto' }).format(Math.round((new Date(item.createdAt).getTime() - Date.now()) / 86400000), 'day')}</time></span></button>)}</div></section>}
