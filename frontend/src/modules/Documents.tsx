...
export function Documents({ initialID = "" }: { initialID?: string }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await api<Document>("/api/v1/documents", {
        method: "POST",
        body: JSON.stringify({
          title: form.get("title"),
          body: form.get("body"),
          links: documentLinks(form),
        }),
      });
      setItems((current) => [created, ...current]);
      setLinkOptions((current) => [
        ...current,
        { type: "document", id: created.id, label: created.title },
      ]);
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setFormError("");
    try {
      const updated = await api<Document>(`/api/v1/documents/${selected.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          title: form.get("title"),
          body: form.get("body"),
      ...
      setEditing(false);
      const history = await api<{ revisions: DocumentRevision[] }>(
        `/api/v1/documents/${selected.id}/revisions`,
      );
                ...
                (editing ? (
                  <form className="document-editor" onSubmit={save}>
                    ...
                    <textarea
                      ...
                      autoFocus
                    />
                    <LinkFields links={selected.links} options={linkOptions} />
                    {formError && (
      ...
      {creating && (
        ...
        >
          <form onSubmit={create}>
            ...
            <LinkFields options={linkOptions} />
            {formError && (
              <p className="form-error" role="alert">
                {formError}
              </p>
            ...
            <div className="form-actions">
              ...
              <button className="primary-button">Create document</button>
            </div>
          </form>
        </Modal>
      )}
...
function LinkFields({
  ...
  const [type, setType] = useState<RecordLink["type"] | "">(
    links[0]?.type ?? "",
  );
    ...
    <div className="field-grid">
      <label>
        ...
        <select
          ...
          value={type}
          onChange={(event) => {
            setType(event.target.value as RecordLink["type"] | "");
            setID("");
          }}
        >
