...
export function Accounts() {
  ...
  async function open(account: Account) {
    try { setSelected(await api<AccountDetail>(`/api/v1/accounts/${account.id}`)) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not open account') }
  }
