...
export function Communications() {
  ...
  return <Page eyebrow="Conversations" title="Communications" detail="Send intentional emails, notice customer replies, and jump into Google Voice without turning Kosmos into another inbox.">
    ...
    <section className="split-grid lower-grid">
      <div className="panel"><p className="eyebrow">Google Voice</p><h2>Call or text a contact</h2><p className="muted-copy">Kosmos opens the native phone or messaging app, or takes you to Google Voice. It does not pretend Google Voice has an automation API.</p><label>Phone number<input value={phone} onChange={(event) => setPhone(event.target.value)} inputMode="tel" placeholder="+1 555 123 4567" /></label><div className="button-row">{phone ? <><a className="secondary-button" href={`tel:${phone}`}>Call</a><a className="secondary-button" href={`sms:${phone}`}>Text</a></> : <><button className="secondary-button" disabled>Call</button><button className="secondary-button" disabled>Text</button></>}<a className="primary-button" href="https://voice.google.com/u/0/messages" target="_blank" rel="noreferrer">Open Google Voice <ExternalLink size={15} /></a></div></div>
